...
class Agent
{
    ...
    void computeNewVelocity()
    {
        ...
        for (final Pair<Double, Obstacle> obstacleNeighbor : obstacleNeighbors)
        {
            ...
            {
                if (RVOMath.det(relativePosition1.scalarMultiply(invTimeHorizonObstacle).subtract(orcaLine.point), orcaLine.direction)
                        - invTimeHorizonObstacle * radius >= -RVOMath.EPSILON
                        && RVOMath.det(relativePosition2.scalarMultiply(invTimeHorizonObstacle).subtract(orcaLine.point), orcaLine.direction)
                                - invTimeHorizonObstacle * radius >= -RVOMath.EPSILON)
                {
                    alreadyCovered = true;
            ...
            if (s < 0.0 && distanceSqLine <= radiusSq)
            {
                ...
                leftLegDirection = new Vector2D(relativePosition1.getX() * leg1 - relativePosition1.getY() * radius,
                        relativePosition1.getX() * radius + relativePosition1.getY() * leg1).scalarMultiply(1.0 / distanceSq1);
                rightLegDirection = new Vector2D(relativePosition1.getX() * leg1 + relativePosition1.getY() * radius,
                        -relativePosition1.getX() * radius + relativePosition1.getY() * leg1).scalarMultiply(1.0 / distanceSq1);
            }
            else if (s > 1.0 && distanceSqLine <= radiusSq)
            {
                ...
                leftLegDirection = new Vector2D(relativePosition2.getX() * leg2 - relativePosition2.getY() * radius,
                        relativePosition2.getX() * radius + relativePosition2.getY() * leg2).scalarMultiply(1.0 / distanceSq2);
                rightLegDirection = new Vector2D(relativePosition2.getX() * leg2 + relativePosition2.getY() * radius,
                        -relativePosition2.getX() * radius + relativePosition2.getY() * leg2).scalarMultiply(1.0 / distanceSq2);
            }
            else
            {
        ...
        for (final Pair<Double, Agent> agentNeighbor : agentNeighbors)
        {
            ...
            if (distanceSq > combinedRadiusSq)
            {
                ...
                else
                {
                    ...
                    {
                        // Project on left leg.
                        direction = new Vector2D(relativePosition.getX() * leg - relativePosition.getY() * combinedRadius,
                                relativePosition.getX() * combinedRadius + relativePosition.getY() * leg).scalarMultiply(1.0 / distanceSq);
                    }
                    else
                    {
                        // Project on right leg.
     ...
     *
     * @param obstacle
     *            The number of the static obstacle to be inserted.
     * @param rangeSq
     *            The squared range around this agent.
     */
     ...
     * @param optimizationVelocity
     *            The optimization velocity.
     * @param optimizeDirection
     ...
     * defined by lines and a circular constraint.
     *
     * @param numObstacleLines
     *            Count of obstacle lines.
     * @param beginLine
     *            The line on which the 2-D linear program failed.
     */
        ...
        for (int i = beginLine; i < lines.size(); i++)
        {
            if (RVOMath.det(lines.get(i).direction, lines.get(i).point.subtract(newVelocity)) > distance)
            {
                ...
                for (int j = numObstacleLines; j < i; j++)
                {
                    ...
                    {
                        point = lines.get(i).point.add(
                                lines.get(i).direction.scalarMultiply(RVOMath.det(lines.get(j).direction, lines.get(i).point.subtract(lines.get(j).point)) / determinant));
                    }
