...
class KdTree
{
     ...
     *
     * @param agent
     *            The agent for which agent neighbors are to be computed.
     * @param rangeSq
     ...
     *
     * @param agent
     *            The agent for which obstacle neighbors are to be computed.
     * @param rangeSq
     *            The squared range around the agent.
     */
     ...
     *
     * @param q1
     *            The first point between which visibility is to be tested.
     * @param q2
     *            The second point between which visibility is to be tested.
     * @param radius
     *            The radius within which visibility is to be tested.
    ...
    private static ObstacleTreeNode buildObstacleTreeRecursive(List<Obstacle> obstacles)
    {
        ...
        for (int j = 0; j < obstacles.size(); j++)
        {
            ...
            {
                // Split obstacle j.
                final double t = RVOMath.det(obstacleI2.point.subtract(obstacleI1.point), obstacleJ1.point.subtract(obstacleI1.point))
                        / RVOMath.det(obstacleI2.point.subtract(obstacleI1.point), obstacleJ1.point.subtract(obstacleJ2.point));
    ...
    private double queryAgentTreeRecursive(Agent agent, double rangeSq, int node)
    {
        ...
        {
            final double distanceSqLeft = sqr(FastMath.max(0.0, agentTree[agentTree[node].left].minX - agent.position.getX()))
                    ...
                    + sqr(FastMath.max(0.0, agent.position.getY() - agentTree[agentTree[node].left].maxY));
            final double distanceSqRight = sqr(FastMath.max(0.0, agentTree[agentTree[node].right].minX - agent.position.getX()))
                    + sqr(FastMath.max(0.0, agent.position.getX() - agentTree[agentTree[node].right].maxX))
                    + sqr(FastMath.max(0.0, agentTree[agentTree[node].right].minY - agent.position.getY()))
                    + sqr(FastMath.max(0.0, agent.position.getY() - agentTree[agentTree[node].right].maxY));
    ...
    private static boolean queryVisibilityRecursive(Vector2D q1, Vector2D q2, double radius, ObstacleTreeNode node)
    {
        ...
        if (q1LeftOfI >= 0.0 && q2LeftOfI >= 0.0)
        {
            return queryVisibilityRecursive(q1, q2, radius, node.left)
                    && (q1LeftOfI * q1LeftOfI * invLengthI >= radius * radius && q2LeftOfI * q2LeftOfI * invLengthI >= radius * radius
        ...
        return point1LeftOfQ * point2LeftOfQ >= 0.0 && point1LeftOfQ * point1LeftOfQ * invLengthQ > radius * radius && point2LeftOfQ * point2LeftOfQ * invLengthQ > radius * radius
                && queryVisibilityRecursive(q1, q2, radius, node.left) && queryVisibilityRecursive(q1, q2, radius, node.right);
    }
}
