...
public class ExecutorPool
{
    ...
    {
        workerService = ThreadPoolUtil.fixedServicePool(workerSize, workerPoolName);

        if (bossSize > 0)
        {
            delayService = ThreadPoolUtil.fixedServicePool(bossSize, bossPoolName);
            ...
            delayService.submit(() -> {
                while (isRunning)
                {
                    ...
                    if (interval < CHECK_INTERVAL)
                    {
                        ...
                        {
                            LogFactory.error(String.format("delay task check exception. ", e));
                        }
                    }
                    else
                    {
                        LogFactory.warn(String.format("delay task check spend too much time. time:%d", interval));
                    }
                }
            });
        }
    ...
    public ExecutorPool(int workerSize, String workerPoolName)
    {
        this(0, workerSize, workerPoolName, "");
    }
