...
{
    protected static final int MAX_INTERVAL = 500;
    ...
    public void run()
    {
        try
        {
            ...
            if (interval > 200)
            {
                LogFactory.debug(String.format("AbstractTask:%s speed too much time:%d.", getName(), interval));
            }
        }
        catch (Exception e)
        {
            LogFactory.error("任务出错：", e);
        }
        finally
        {
            this.queue.complete();
