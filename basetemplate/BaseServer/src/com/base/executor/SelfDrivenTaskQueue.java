...
{
    private static int MAX_OVER_TASK = 50;

    private static int MAX_STOP_TASK = 200;
                ...
                if (this.taskQueue.size() > MAX_OVER_TASK)
                {
                    if (this.taskQueue.peek() != null)
                    {
                        if (this.taskQueue.peek() instanceof AbstractTask)
                        {
                            ...
                            LogFactory.error("SelfDrivenTaskQueue Add Count Over 100.size return.:{} - executing name:{}",
