...
public abstract class BaseDao<T>
{
    ...
    {
        T result = getDBHelper().executeQuery(sql, null,
                new DataReader<T>()
                {
                    @Override
    ...
    {
        DBParamWrapper dbParamWrapper = new DBParamWrapper();
        for (Object p : params)
            dbParamWrapper.put(p);

        return query(sql, dbParamWrapper);
    }
    ...
    {
        List<T> entitis = getDBHelper().executeQuery(sql, null,
    ...
    {
        DBParamWrapper paramWrapper = new DBParamWrapper();
        for (Object p : params)
            paramWrapper.put(p);
        return queryList(sql, paramWrapper);
    }
    ...
    {
        Map<K, T> resultMap = getDBHelper().executeQuery(sql, paramWrapper, new DataReader<Map<K, T>>()
        {
            @Override
    ...
    protected List<T> rsToEntityList(ResultSet rs)
    {
        ...
        if (rs != null)
        {
            ...
            {
                LogFactory.error("Resultset转成实体出错", e);
            }
        }
        return entitis;
    }
