...
public class CommonMessage implements Serializable
{
    ...
    public static CommonMessage build(byte[] msgData)
    {
        ...
        if (bodyLen > 0)
        {
            ...
            {
                LOGGER.error("数据包超过最大长度。max:{},len:{}.", Short.MAX_VALUE, bodyLen);
                return null;
            }
            ...
            if (builder.checksum != getCS)
            {
                ...
                LOGGER.warn("数据包校验失败，数据包将被丢弃。code: 0x{}。校验和应为{}，实际接收校验和为{}", builder.getCode(), getCS,
                        builder.checksum);
                return null;
            }
        }
    ...
    public ByteBuf toByteBuf()
    {
        ...
        buf.setIndex(0, 6);
        buf.writeShort(this.getCode());
        buf.writeInt(this.getParam());
        ...
        // 插入校验和
        buf.setIndex(0, 4);
        buf.writeShort(calcChecksum(buf.array()));
    ...
    public short getLen()
    {
        if (bodyData != null)
        {
            if (bodyData.length > 32750)
                LOGGER.error("数据包超过short类型最大值！！- " + bodyData.length);
    ...
    {
        CommonMessage clone = new CommonMessage();
        clone.setCode(this.code);
        clone.setBody(this.bodyData);
